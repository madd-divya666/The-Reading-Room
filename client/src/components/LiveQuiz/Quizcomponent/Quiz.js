...
const Quiz = ({ showQuiz, question, quizs, checkAnswer, correctAnswer, selectedAnswer, questionIndex, nextQuestion, showTheResult }) => {
    ...
    return (
        <section className=" text-white" style={{
            display: `${showQuiz ? 'block' : 'none'}`, backgroundColor: '#ffdee9', backgroundImage: 'linear-gradient(0deg, #ffdee9 0%, #f3a05d 100%)'
        }}>
            <div className="container">
                <div className="row vh-100 align-items-center justify-content-center">
                    <div className="col-lg-8">
                        <div className="card p-4" style={{ background: '#ff8800', borderColor: '#646464' }}>
                            <div className="d-flex justify-content-between gap-md-3">
                                <h5 className='mb-2 fs-normal lh-base'>{question?.statement}</h5>
                            ...
                            <div>
                                {
                                    ...
                                    question?.options?.map((item, index) => <button
                                        key={index}
                                        className={`option w-100 text-start btn text-black py-2 px-3 mt-3 rounded btn-light ${correctAnswer === item && 'bg-success'}`}
